...
return [
    ...
    [
        'id' => 'AUD-SEC-011',
        'name' => 'Weak or missing CSRF protection',
        'domain' => 'security',
        'severity' => 'high',
        'confidence' => 'medium',
        'description' => 'CSRF protection is disabled or bypassed for a state-changing route: `VerifyCsrfToken::except` whitelists, CSRF middleware is removed or reordered, or forms/post endpoints miss the CSRF token.',
        'why_it_matters' => 'Without CSRF protection, a logged-in user can be tricked into submitting a state-changing request from another site, enabling account takeover and data tampering.',
        'recommendation' => 'Keep the CSRF middleware enabled for all state-changing routes, remove `VerifyCsrfToken::except` entries (prefer token-based auth for APIs), and ensure forms include `@csrf` or `axios`/`fetch` XSRF handling.',
        'evidence' => [
            'The route and middleware that should enforce CSRF.',
            'The `bootstrap/app.php` middleware configuration or `VerifyCsrfToken::except` list.',
            'The form/token handling that sends or omits the CSRF token.',
        ],
        'false_positive_considerations' => [
            'Stateless APIs using token authentication (Sanctum, OAuth) do not need cookie CSRF protection.',
            'The route may be handled by an external service or webhook that sends a shared secret instead.',
        ],
        'references' => [
    ...
    [
        'id' => 'AUD-SEC-012',
        'name' => 'Unescaped output or XSS risk',
        'domain' => 'security',
        'severity' => 'high',
        'confidence' => 'medium',
        'description' => 'User-controlled or database-stored data is rendered without escaping (Blade `{!! !!}`, `->get()`, `->toHtml()`, or `htmlspecialchars` bypass), allowing stored or reflected cross-site scripting.',
        'why_it_matters' => 'XSS lets an attacker run scripts in another user\'s session, steal cookies, or perform actions as the victim.',
        'recommendation' => 'Escape output with `{{ }}` by default, sanitize or allow-list HTML when unescaped rendering is intentional, and validate/encode context-appropriately for JavaScript, CSS, and attribute contexts.',
        'evidence' => [
            'The view or render path that outputs unescaped data.',
            'The source of the value (user input, database, third-party API).',
        ],
        'false_positive_considerations' => [
            'The value may be trusted application content (e.g. an admin-authored blog post rendered through a validated rich-text pipeline).',
            'A sanitization layer may already strip or encode dangerous markup before it reaches the view.',
        ],
        'references' => [
    ...
    [
        'id' => 'AUD-SEC-013',
        'name' => 'Raw SQL with user-controlled input',
        'domain' => 'security',
        'severity' => 'high',
        'confidence' => 'medium',
        'description' => 'Raw SQL expressions (`DB::raw`, `whereRaw`, `selectRaw`, `orderByRaw`, `havingRaw`, or `statement`/`select`) interpolate user-controlled input or unvalidated values.',
        'why_it_matters' => 'Unescaped input in raw SQL can enable SQL injection, letting an attacker read, modify, or delete data beyond their authorization.',
        'recommendation' => 'Use the query builder or Eloquent with bound parameters wherever possible; when raw SQL is unavoidable, bind values with `?` placeholders or the driver\'s escaping and validate/normalize any identifiers.',
        'evidence' => [
            'The raw SQL call and the value interpolated into it.',
            'The origin of that value (request input, query string, file, third-party API).',
        ],
        'false_positive_considerations' => [
            'The value may be bound via the query builder and only the SQL fragment itself is raw.',
            'A constant, fully-trusted value (e.g. a hardcoded fragment) carries no injection risk.',
        ],
        'references' => [
